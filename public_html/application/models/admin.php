			...
			if ($query){
                ...
                public function misPreCanjes(){
                        $query=$this->db->query("
                                ...
                                dsMov as Nombre_Esp, '-' as Status,'-' as Mensajeria,'-' as NumeroGuia,noPuntos as puntos from PartMovsRealizados p  where p.idParticipante = ".$this->session->userdata('idPart')."
                                ORDER BY 2,1
                        ");
                        if ($query->num_rows() > 0){
                                return $query->result_array(); 
                ...
                public function misCanjes(){
    		        $query = $this->db->query("
                                ...
                                pr.Nombre_Esp as Descripcion,pr.Marca,pd.PuntosXUnidad as Puntos,pd.PuntosXUnidad*pd.Cantidad as Total
                                FROM PreCanje pc
